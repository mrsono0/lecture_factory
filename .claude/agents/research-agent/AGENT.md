...
동작:
1. `deep_research_plan.md`의 요청별 심화 질문으로 로컬 파일 탐색
2. Phase 2 Step 1과 동일한 확장자별 읽기 전략 적용:
   - PDF ≤20p: `Read(pages="1-20")` (1순위)
   - PDF >20p: `/opt/homebrew/bin/pdftotext "{file}" -` (2순위)
   - PDF Read 실패 시: `source .venv/bin/activate && python -c "import pdfplumber; ..."` (폴백)
3. 요청과 관련된 내용 발견 시 인용 번호 부여 + 발견 내용 기록
...
**교수법 효과성 3중 검증 (교안 Phase 4 신규 — 위 기본 검증에 추가 적용)**:
...
|-----------|----------------------|---------------------|
| 강의구성안 | 위 Phase 2 Step 0~4 전체 수행 | 위 "강의구성안 심화 리서치 (Phase 4)" Step 0~2 전체 수행 (deep-research 스킬 기반) |
| 강의교안 | 위 "강의교안 탐색적 리서치 (Phase 2)" Step 0~4 전체 수행 | 위 "강의교안 심화 리서치 (Phase 4)" Step 0~2 전체 수행 (교수법 효과성 검증 특화) |
