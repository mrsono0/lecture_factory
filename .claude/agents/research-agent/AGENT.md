...
Step 0: 입력 변환 + 심화 리서치 계획 수립
  ...
  │     ├── Retrieve: WebSearch + WebFetch + (조건부) 로컬/NBLM 재조회
...
3. 각 요청을 deep-research 입력 형식으로 변환:
   ...
   - 우선순위 → 처리 순서 + 검색 예산 배분
4. 자료원 재활용 판단:
   - 로컬 참고자료: 요청 내용이 로컬 자료에서 답변 가능한 경우만 재조회
   - NotebookLM: 요청 내용이 사용자 선별 소스에서 심화 가능한 경우만 재쿼리
...
| 도구 | Read, WebSearch, WebFetch, Write (+ Glob, Bash: 로컬/NBLM 재조회 시) |
...
- WebSearch로 검색 실행 (검색 예산 내)
- 검색 결과에서 고품질 소스 선별 → WebFetch로 상세 내용 수집
- (조건부) 로컬 참고자료 재조회: Phase 2에서 분석한 파일을 새로운 질문으로 재분석
- (조건부) NBLM 재쿼리: 심화 질문으로 사용자 소스 재조회

**Anti-Hallucination Protocol (필수)**:
- 수집된 모든 팩트에 즉시 인용 번호 `[N]` 부여
- "According to [N]..." 또는 "[N]에 따르면..." 형식 사용
- 소스에서 확인할 수 없는 내용은 절대 인용하지 않음
- 불확실하면 "소스를 찾지 못함"으로 명시

#### 1-4. Triangulate (삼각 검증) — 검증 유형 필수

| 상황 | 태그 | 처리 |
|------|------|------|
| 3+ 독립 소스 일치 | `[확정]` | 확정 사실로 채택 |
| 2 소스 일치, 1 불일치 | `[높은 확신]` | 다수 의견 채택, 불일치 기록 |
| 소스 간 모순 | `[불일치]` | 양쪽 기록, 추가 검색 시도 |
| 1 소스만 존재 | `[미검증]` | 출처 명시, 추가 검증 필요 표시 |
...
| # | 출처 | 유형 | 접근일자 | 신뢰도 |
|---|------|------|---------|--------|
| [1] | {URL/파일경로} | 웹/로컬/NBLM | {날짜} | [확정/높은 확신/미검증] |
```
