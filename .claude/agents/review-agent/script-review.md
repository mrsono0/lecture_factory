...
> **GATE-6과의 역할 분담**: GATE-6(bash 스크립트)은 정량 검증(줄 수, 코드 블록 수, 금지 문구)을 자동 수행한다. Phase 7 review-agent는 GATE-6을 통과한 산출물에 대해 콘텐츠 품질·교수설계 적합성·입력 충실성 등 정성 검증을 수행한다. G-1~G-4는 GATE-6과 일부 중복되나 에이전트 관점의 재검증으로서 유지한다.
...
| 4 | G-4 | GRR 시간 비율 현실성 | 직접교수법 기준: I Do 40~55%, We Do 28~38%, You Do 12~25%. 교수 모델별 기준 적용 | Minor (기준 범위 소폭 이탈) |
...
| 3 | T-3 | GRR 시간 비율 현실성 | 직접교수법 기준: I Do 40~55%, We Do 28~38%, You Do 12~25%. 교수 모델별 기준 적용 (PBL: We Do 중심, 플립: You Do 중심) | Minor (기준 범위 소폭 이탈) |
...
| 6 | T-6 | 활동 시간 명시 | 모든 활동 지시에 소요 시간 명시 (`({N}분)` 형태). 교안에서는 이모지 대신 텍스트 헤딩으로 활동을 식별 | Minor (시간 미명시 활동) |
| 7 | T-7 | 시간 예산 일치 | 교시 헤더의 교수 모델 및 시간 구조 값이 input_data.json의 teaching_model, time_ratio와 일치 | Major (불일치) |
| 8 | T-8 | 시간큐 연속성 | 각 GRR 구간 헤딩의 `({분}분)` 값이 교시 내에서 시간 순서대로 배치되며, 합산이 session_minutes와 ±2분 이내 | Minor (시간큐 역행 또는 누락) |
...
| # | 검증 ID | 검증 항목 | 기준 | 위반 유형 |
|---|---------|----------|------|----------|
| 1 | S-1 | 7개 섹션 존재 | §1~§5, §7~§8 헤딩 모두 존재. 검증 대상: `_header.md`(§1~§3) + `block_D{day}_{AM\|PM}.md`(§4) + `_footer.md`(§5, §7~§8). §6 발문 모음은 교안 콘텐츠 전용 구조에서 제외됨 | Major (누락 섹션당) |
| 2 | S-2 | 메타데이터 완전성 | 작성일자, 입력소스, 교수 모델, 적용 프레임워크 모두 기재 | Minor |
| 3 | S-3 | 필수 테이블 존재 | §1-1 기본정보, §2-1 CLO-SLO매핑, §2-2 차시-SLO매트릭스, §5-1 형성평가일람, §8-1 오개념카드 | Major (누락 테이블당) |
...
| 6 | S-6 | 서브섹션 완전성 | §1(1-1~1-3), §2(2-1~2-2), §3(3-1~3-3), §5(5-1~5-3), §7(7-1~7-3), §8(8-1~8-5) 총 19개 서브섹션 모두 존재 | Major (누락 서브섹션당) |
