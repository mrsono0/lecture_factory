...
4. **3단계 판정**: `shared/judgment-criteria.md` 참조
...
**점진적 Write 패턴**: Step 1~4 각각의 검증 결과를 중간 파일로 Write한 뒤, Step 5에서 Read하여 통합한다. 5개 입력 파일 + 38개 검증 결과를 동시에 메모리에 유지하는 부담을 줄여 정확도를 높인다.
...
| S-6 | 서브섹션 완전성 | §1(1-1~1-4), §2(2-1~2-2), §3(3-1~3-2), §5(5-1~5-3), §6(6-1~6-4), §7(7-1~7-3), §8(8-1~8-3), §9(9-1~9-4) 총 25개 서브섹션 모두 존재 | Major (누락 서브섹션당) |
...
- 검증 항목 수: 38개 (S:6 + L:5 + A:5 + F:4 + T:7 + C:11)
...
38개 검증 항목 전체의 Pass/Fail 결과를 기록한다.
...
| ... | ... | ... | ... | ... |
| 38 | C-11 | 콘텐츠 정확성 | {Pass/Major/Minor} | {검증 결과 요약} |
