...
**점진적 Write 패턴**: Step 1~4 각각의 검증 결과를 중간 파일로 Write한 뒤, Step 5에서 Read하여 통합한다. 5개 입력 파일 + 36개 검증 결과를 동시에 메모리에 유지하는 부담을 줄여 정확도를 높인다.
...
36개 검증 항목 전체의 Pass/Fail 결과를 기록한다.
...
3. **45개 항목, 6개 영역**: 핵심에 집중하되 교수법 검증을 충분히 커버
...
| 검증 항목 수 | 36개 (S:6 + L:5 + A:5 + F:4 + T:7 + C:9) | **45개** (S:7 + G:8 + P:7 + T:8 + C:8 + N:7) |
...
Step 0: 입력 로드
  ...
  │   └── Write → _review_step4.md
  │
  ├── Step 5: 콘텐츠 정확성 검증 — Anti-Hallucination (C-1~C-8, 8항목)
  │   └── Write → _review_step5.md
  │
  ├── Step 6: 교안 실행 품질 검증 — 발화문/활동지시/강사가이드 (N-1~N-7, 7항목)
  │   └── Write → _review_step6.md
  │
  └── Step 7: Read _review_step1~6.md → 통합 판정 → Write quality_review.md
```

**점진적 Write 패턴**: Step 1~6 각각의 검증 결과를 중간 파일로 Write한 뒤, Step 7에서 Read하여 통합한다. 7개 입력 파일 + 45개 검증 결과를 동시에 메모리에 유지하는 부담을 줄여 정확도를 높인다.
...
5. Step 5: 콘텐츠 정확성 검증 (해당 블록 세션만, C-1~C-8)
6. Step 6: 교안 실행 품질 검증 (해당 블록 세션만, N-1~N-7)
...
| 판정 | 조건 | 후속 조치 |
|------|------|----------|
| **PASS** | Major 위반 0개 + Minor 위반 3개 이하 | Phase 7 종료, lecture_script.md 확정 |
| **CONDITIONAL PASS** | Major 위반 0개 + Minor 위반 4개 이상 | Minor 위반 목록 제공, 부분 수정 권고 |
| **REVISION REQUIRED** | Major 위반 1개 이상 | Major 위반 목록 + 수정 가이드 제공, 해당 교시 재작성 필요 |
...
구성안과 동일. 모든 위반 사항에 다음 형식의 근거 인용을 **필수** 포함한다:

| 요소 | 설명 |
|------|------|
| **위반 내용** | 구체적으로 무엇이 잘못되었는지 기술 |
| **기대값** | 원본 파일에서의 정확한 값 (파일명 §섹션 명시) |
| **실제값** | lecture_script.md에서의 실제 값 (§섹션, 교시 명시) |
| **수정 가이드** | 어떻게 고쳐야 하는지 구체적 방법 제시 |
...
| 5 | T-5 | 발화 블록 길이 | 단일 발화 블록(`> "..."` 연속)이 3~5문장/블록 이내. 15분 초과 연속 발화 없음 | Minor (5문장 초과 블록 존재) |
...
**동작**: C-1~C-8 검증 결과를 `_review_step5.md`에 Write한다.
...
| # | 검증 ID | 검증 항목 | 기준 | 위반 유형 |
|---|---------|----------|------|----------|
| 1 | N-1 | 발화문 자연성 | 발화문(`> "..."`)이 한국어 구어체로 작성되었는지. 3~5문장/블록 이내. 문어체나 학술 용어만 사용한 발화 검출 | Minor (문어체 블록 — 교시당) |
...
**동작**: N-1~N-7 검증 결과를 `_review_step6.md`에 Write한다.
...
| 검증 항목 수 | 45개 (S:7 + G:8 + P:7 + T:8 + C:8 + N:7) |
| 판정 | {PASS / CONDITIONAL PASS / REVISION REQUIRED} |
...
45개 검증 항목 전체의 Pass/Fail 결과를 기록한다.
...
| 2 | S-2 | 구조 완전성 | {Pass/Major/Minor} | {검증 결과 요약} |
| ... | ... | ... | ... | ... |
| 45 | N-7 | 교안 실행 품질 | {Pass/Major/Minor} | {검증 결과 요약} |
