...
description: 강의구성안 생성 - 8단계 파이프라인 (입력수집 → 탐색리서치 → 브레인스토밍 → 심화리서치 → 아키텍처 → 작성 → 검토 → 거시적 구성안)
context: fork
...
**지시사항**: `.claude/agents/writer-agent/AGENT.md`를 읽고 라우팅에 따라 `outline-write.md`를 로드하여 따르세요.
...
**완료 확인**: `{output_dir}/lecture_outline.md` 존재 + Grep으로 §1~§9 전체 섹션 헤더 존재 확인
...
**지시사항**: `.claude/agents/writer-agent/AGENT.md`를 읽고 라우팅에 따라 `outline-write.md`를 로드하여 따르되, **Step 0 + Step 1 + Step 2만 실행**하세요.
...
**지시사항**: `.claude/agents/writer-agent/AGENT.md`를 읽고 라우팅에 따라 `outline-write.md`를 로드하여 따르되, **Step 3의 §5-1, §5-2, §5-3 중 Day 1~Day {mid}만 실행**하세요.
...
**지시사항**: `.claude/agents/writer-agent/AGENT.md`를 읽고 라우팅에 따라 `outline-write.md`를 로드하여 따르되, **Step 3의 §5-3 중 Day {mid+1}~Day {days} + Step 4 + Step 5를 실행**하세요.
...
**지시사항**: `.claude/agents/review-agent/AGENT.md`를 읽고 라우팅에 따라 `outline-review.md`를 로드하여 따르세요.
...
Phase 7 종료. **Phase 8(거시적 구성안 생성)을 실행한 후** 사용자에게 검토 요약을 보고한다:
- 판정: PASS
- Pass/Fail 통계 (Major/Minor 위반 수)
- 우수 사항 (§5에서 추출)
- 최종 산출물 경로: `{output_dir}/lecture_outline.md`, `{output_dir}/macro_outline.md`
...
3. 수정 선택 시 → writer-agent 재호출:
   - **subagent_type**: `writer-agent`
   - **prompt**: `quality_review.md`의 Minor 위반 목록 + 수정 권고를 포함하여 `{output_dir}/lecture_outline.md` 부분 수정 지시. `.claude/agents/writer-agent/AGENT.md`를 읽고 라우팅에 따라 `outline-write.md`를 로드하여 따르되, 수정 범위는 위반 항목에 한정.
4. 수정 완료 후 → review-agent 재호출 (Phase 7 재실행, **최대 1회**)
5. 재검토 결과 확인 후 **Phase 8(거시적 구성안 생성)을 실행**. 사용자에게 보고 (재검토 후에는 판정과 무관하게 종료)

> "현재 상태로 확정합니다" 선택 시에도 **Phase 8을 실행**한 후 종료한다.
...
3. 수정 선택 시 → writer-agent 재호출:
   - **subagent_type**: `writer-agent`
   - **prompt**: `quality_review.md`의 Major 위반 수정 가이드를 포함하여 `{output_dir}/lecture_outline.md` 해당 섹션 재작성 지시. `.claude/agents/writer-agent/AGENT.md`를 읽고 라우팅에 따라 `outline-write.md`를 로드하여 따르되, 수정 범위는 위반 항목에 한정.
4. 재작성 완료 후 → review-agent 재호출 (Phase 7 재실행, **최대 1회**)
5. 재검토 결과 확인 후 **Phase 8(거시적 구성안 생성)을 실행**. 사용자에게 보고 (재검토 후에는 판정과 무관하게 종료)

> "현재 상태로 보존합니다" 선택 시에도 **Phase 8을 실행**한 후 종료한다.

---

### Phase 8: 거시적 강의구성안 생성

Phase 7 후속 처리의 모든 분기가 완료된 직후 실행한다. lecture_outline.md의 데이터를 요약·변환하여 과정 전체를 한 페이지 테이블로 조망하는 거시적 구성안을 생성한다.
...
```
최종 확정된 강의구성안을 기반으로 거시적 강의구성안(한 페이지 테이블 요약)을 생성하세요.

**지시사항**: `.claude/agents/writer-agent/AGENT.md`를 읽고 라우팅에 따라 `macro-outline-write.md`를 로드하여 따르세요.

**입력**: `{output_dir}/lecture_outline.md`, `{output_dir}/input_data.json`
**템플릿**: `.claude/templates/macro-outline-template.md`
**산출물**: `{output_dir}/macro_outline.md`
**제약**: 도구 Read, Write만 사용. 외부 검색 없음. Agent 중첩 금지.
**금지**: lecture_outline.md에 없는 새로운 모듈/주제/시간 배치 창작 금지. 교시 병합 시 원본 시간 합계를 초과하는 시간 배정 금지.
```

**완료 확인**: `{output_dir}/macro_outline.md` 존재 + 테이블에 모든 Day가 포함되어 있는지 확인

**실패 처리**: 산출물 생성 실패 시 사용자에게 경고 후 워크플로우를 종료한다. `lecture_outline.md`는 이미 확정된 상태이므로 Phase 8 실패가 워크플로우 전체를 무효화하지 않는다.
...
**상세**: `.claude/agents/writer-agent/AGENT.md` 라우팅 → `outline-write.md` 참조

### Phase 7: 품질 검토 → review-agent

**상세**: `.claude/agents/review-agent/AGENT.md` 라우팅 → `outline-review.md` 참조

### Phase 8: 거시적 강의구성안 생성 → writer-agent

**상세**: `.claude/agents/writer-agent/AGENT.md` 라우팅 → `macro-outline-write.md` 참조
...
├── lecture_outline.md           # Phase 6: 최종 구성안 (미시적) ★
...
└── macro_outline.md             # Phase 8: 거시적 강의구성안 ★
```
