...
/lecture-outline (Skill = 오케스트레이터)
     ...
     └── Phase 8: Agent → writer-agent (거시적 구성안)
```
...
| 8 | 거시적 강의구성안 | writer-agent | 과정 전체를 한 페이지 테이블로 요약 (교시→시간 블록 병합, 카테고리/태그 매핑) |
...
| 6 | 차시별 교안 작성 | writer-agent | **세션(1교시) 단위 호출** + 차시별 독립 파일(`session_D{day}-{num}.md`) 작성. 교안 콘텐츠 전용(발화문 제외). 오케스트레이터가 입력 발췌 전달 + bash GATE 검증(`gate-check-session.sh`) |
...
```
직접교수법:  도입 16% (8분) / 전개 74% (37분) / 정리 10% (5분)  (GRR: I Do → We Do → You Do)
PBL:        도입 14% (7분) / 전개 78% (39분) / 정리 8% (4분)  (GRR: We Do Together 중심)
플립러닝:    도입 10% (5분) / 전개 82% (41분) / 정리 8% (4분)  (GRR: We Do → You Do Together)
혼합:       도입 14% (7분) / 전개 76% (38분) / 정리 10% (5분)  (차시별 모델 비율 적용)
```
...
| S4 | 시간 비율 | S1a 교수 모델에서 자동 파생 (직접교수: 16/74/10, PBL: 14/78/8 등) |
...
|-----------|------|------|------|---------|
| 직접교수법 | 16% (8분) | 74% (37분) | 10% (5분) | I Do → We Do → You Do |
| PBL | 14% (7분) | 78% (39분) | 8% (4분) | We Do Together 중심 |
| 플립러닝 | 10% (5분) | 82% (41분) | 8% (4분) | We Do → You Do Together |
...
#### Phase 6: 차시별 교안 콘텐츠 작성 — 세션 단위 호출 + bash GATE 검증

Phase 1~5 산출물을 통합하여 학습 콘텐츠를 생성합니다. 강사 발화문은 Phase 8에서 별도 생성합니다.

**교안 콘텐츠 요소**: 개념 설명문(문어체), 다중 예시(★핵심/◇심화), 코드 블록(hands-on), 비유·시각화(Mermaid), 오개념 교정 테이블, 스캐폴드 코드(We Do), 완성 코드(You Do)

**핵심개념 상세도 가이드** (`script-content.md` §3-3b): I Do 핵심개념은 "행동 지시(~를 시연한다)"가 아니라 강사가 실제로 설명할 내용 전체를 담아야 한다. 좋은/나쁜 예시 2종(설치 차시·개념 설명 차시)을 참고하여 작성한다.

**세션 단위 호출**: writer-agent를 **1교시씩** 호출하여 1개 `session_D{day}-{num}.md` 파일만 Write합니다.
- 호출 횟수: 세션 수 + 2 (header + footer)
- 오케스트레이터가 해당 교시에 필요한 입력(architecture, brainstorm, context7)을 **발췌하여 prompt에 인라인 전달**
- 블록별 Context7 보강 쿼리는 hands-on 차시에 대해 세션 호출 직전 수행

**금지 패턴** (GATE-6 bash 스크립트로 자동 검증):

| 금지 문구 | 대체 방법 |
|---------|---------|
| `구성안 비유 체계 참조.` | brainstorm §4의 비유를 5줄 이상 설명문으로 전개 |
| `선수 확인 질문` | 구체적 질문 내용과 예상 답변 작성 |
| `강사 시연을 따라 코드 작성` | 실제 단계별 코드 가이드(스캐폴드 코드 포함) |
| `다음 차시 주제와의 연결점을 안내합니다.` | 구체적 연결 내용 작성 |
| `해당 SLO 달성 여부 확인` | 구체적 성공 기준 작성 |

**최소 분량 기준** (GATE-6 검증):

| content_type | 최소 줄 수 | 필수 요소 |
|-------------|----------|---------|
| hands-on | ≥ 150줄 | 코드 블록 ≥ 3쌍, ★ 예시 ≥ 2개 |
| concept | ≥ 120줄 | ★ 예시 ≥ 2개, 설명문 ≥ 15줄 |
| activity | ≥ 100줄 | 활동 지시문 ≥ 5줄, 성공 기준 |

**setup subtype** (`script-content.md` §3-3c): content_type은 `hands-on` 유지, 도구 설치·환경 구성이 주 내용인 차시에 `subtype=setup`을 지정하여 추가 검증. 필수 포함 요소: URL ≥1, 단계별 설치 과정 ≥4단계, UI 요소 설명(각 2줄+), 명령어+예상 출력, 트러블슈팅 ≥1건. I Do 핵심개념 최소 15줄.

**GATE-6** (bash 스크립트 `.claude/scripts/gate-check-session.sh` 실행):

| ID | 검증 내용 | 기준 | 실패 시 |
|----|----------|------|--------|
| G6-7 | 금지 문구 미존재 | 0건 | 해당 교시 재작성 |
| G6-8 | hands-on 코드 블록 | ≥ 6 fence(3쌍) | 해당 교시 재작성 |
| G6-9 | session 줄 수 하한 | content_type별 기준 | 해당 교시 재작성 |
| G6-10 | setup subtype 필수 요소 | URL ≥1, 예상 출력 ≥1, 단계 ≥4 | 해당 교시 재작성 |
| G6-11 | I Do 핵심개념 섹션 줄 수 | setup ≥15줄, hands-on/concept ≥8줄, 기타 ≥5줄 | 해당 교시 재작성 |
| G6-12 | 오개념 교정 테이블 데이터 행 | ≥ 1행 (템플릿 잔존 불가) | 해당 교시 재작성 |

**산출물 파일**:
- `_header.md`: §1~§3 개요·목표·공통구조 (Part 0)
- `session_D{day}-{num}.md`: 차시별 독립 교안 콘텐츠 (각 세션 1파일)
- `_footer.md`: §5~§8 형성평가·참고자료·강사가이드 (Part N)

상세 워크플로우: `.claude/agents/writer-agent/AGENT.md` 라우팅 → `script-content.md` 참조
...
| 검증 항목 수 | 38개 (6영역) | 29개 (4영역: G+T+C+EX) | ~15개 (S+일관성) |
| Step 수 | Step 0~5 | 블록 수만큼 반복 | 1회 |
| 재작성 | 없음 | REVISION 시 writer-agent 재호출 (최대 1회) | 없음 |
| 산출물 | `quality_review.md` | `_review_content_{block_id}.md` | `block_D{day}_{AM|PM}.md` (블록별 통합) + `quality_review.md` |

**블록별 검토 루프**:

```
for block in blocks:
    1. review-agent 호출 (해당 블록의 session_*.md 파일들, 29항목)
       → _review_content_{block_id}.md
...
**GATE-6과의 역할 분담**: GATE-6(bash 스크립트)은 정량 검증(줄 수, 코드 블록 수, 금지 문구)을 자동 수행. Phase 7 review-agent는 GATE-6 통과 산출물에 대해 콘텐츠 품질·교수설계 적합성·입력 충실성 등 정성 검증 수행. G-1~G-4는 GATE-6과 일부 중복되나 에이전트 관점 재검증으로 유지.

**블록별 검증 4영역 (29항목)** — review-agent (opus 모델):

| 영역 | ID 범위 | 항목 수 | 주요 검증 기준 |
|------|---------|--------|--------------|
| G: 구조 완전성 | G-1~G-4, G-7 | 5 | 5구간 완전성, GRR 순차, GRR 시간 배분, 확인 활동 삽입 |
| T: 시간 배분 | T-1~T-8 | 8 | 교시 합산, time_ratio ±5%, GRR 비율, 15분 분절, GRR 구간 헤딩 시간 연속성 (T-6/T-8: 텍스트 기반 검증, 이모지 미사용) |
| C: 콘텐츠 정확성 | C-1~C-12 | 12 | CLO/SLO 대조, 소재 근거, 코드 API 정확성(C-9), 코드 밀도(C-10), 플레이스홀더(C-11~C-12) |
| EX: 예시 품질 | EX-1~EX-4 | 4 | ★ 예시 ≥ 2개(EX-1), You Do 완전성(EX-2), 오개념 반영(EX-4) |

**GATE-7 추가 항목**:

| ID | 검증 내용 | 기준 | 실패 시 |
|----|----------|------|--------|
| G7-4 | review 검증 항목 수 ≥ 29 | 산출물 내 검증 ID 행 카운트 | review-agent 재호출 |

> **[CRITICAL]** review-agent 모델이 opus로 변경됨. 29개 항목 전수 검증을 강제하며, 항목 수 미달 시 GATE-7 FAIL 처리.
...
#### Phase 8: 강사대본 생성 — 교안 기반 발화문 변환 + bash GATE 검증

검토 완료된 session 교안을 기반으로 강사가 실제 말할 발화문을 생성합니다.

**대본 생성 원칙**:
- 교안 참조 기반: 교안의 설명을 구어체 발화문으로 변환. 교안에 없는 팩트 창작 금지
- 시간 관리: ★ 핵심 예시 반드시 포함, ◇ 심화 예시는 `[시간 여유 시]` 선택 배치
- 대본 전용 표기: `> "..."` 발화문, `❓` 발문, `💬` 예상 반응, `[...]` 행동 지시

**금지 패턴** (GATE-8 bash 스크립트 `gate-check-narration.sh` 검증):
- `___` 빈칸, `(강사가 핵심 개념을 설명하고 코드를 시연하는 구간)` 같은 범용 지시문
- 모든 차시 동일 발화문
- 최소 분량: ≥ 80줄, 발화문(`> "`) ≥ 5개, 발문(`❓`) ≥ 2개

**narration GRR 헤딩 표준 규격**: Phase 10 병합 스크립트가 정규표현식(`^#{2,3} (도입|I Do|We Do|You Do|정리)`)으로 파싱. `## 전개` 래퍼 사용 금지 — 각 GRR 구간을 독립 H2(`##`)로 작성. 특수 차시(발표/활동)에서도 `## You Do`로 표기. 상세: `script-narration.md` 헤딩 규격 섹션 참조.

**교안 빈약 시 폴백**: session_*.md의 설명문이 5줄 미만이면 architecture.md, brainstorm_result.md, context7_reference.md에서 직접 소재 활용

**GATE-8** (bash 스크립트 검증):

| ID | 검증 내용 | 기준 | 실패 시 |
|----|----------|------|--------|
| G8-3 | narration `___` 빈칙 | 0건 | 해당 교시 재작성 |
| G8-4 | narration 줄 수 | ≥ 80 | 해당 교시 재작성 |

상세 워크플로우: `.claude/agents/writer-agent/AGENT.md` 라우팅 → `script-narration.md` 참조

#### Phase 9: 대본 품질 검토 — 8항목 검증

대본의 교안 참조 정합, 발화문 구어체, 발문/전환 존재, 팩트 창작 검출 등 8항목(NR-1~NR-8) 검증.

상세 워크플로우: `.claude/agents/review-agent/AGENT.md` 라우팅 → `script-review.md` 대본 검토 모드 참조
...
**Step 10-1: 블록 통합 — 스크립트 실행** (session + narration → block):

Phase 6의 교안 파일과 Phase 8의 대본 파일을 블록 단위로 통합합니다.

**[CRITICAL] 오케스트레이터가 Bash 도구로 병합 스크립트를 실행한다. worker-agent로 병합하지 않는다.**

```bash
python3 .claude/scripts/merge-blocks.py {output_dir} all
```

블록 병합은 텍스트 구조 변환이며 블록당 입력이 90~170KB로 에이전트의 출력 토큰 한계를 초과한다. LLM 에이전트에 위임하면 대본 누락, 인터리빙 순서 오류, 파일 잘림이 발생한다.

**스크립트 동작**:
- `session_D*.md` glob으로 세션 목록 자동 수집 → Day별 AM/PM 블록 자동 분할
- 각 세션의 교안과 대본을 **GRR 구간별 인터리빙 병합**: 교안의 각 구간(도입/I Do/We Do/You Do/정리) 바로 아래에 해당 대본을 `> 🎤` 인용 블록으로 삽입
- 비표준 narration 헤딩("발표 진행", "전개" 등)은 **폴백 매핑**으로 자동 매칭
- `block_D{day}_{AM|PM}.md`로 Write (블록 헤더 + 차시별 교안+대본 통합)
- 강사가 각 GRR 구간에서 교안 내용과 발화 대본을 **동시에 참조**할 수 있는 독립 실행 문서
- `_header.md`(§1~§3)와 `_footer.md`(§5~§8)는 block 파일에 포함하지 않는다 — 별도 참조 문서로 유지

**GATE-10-1 (블록 병합 검증, 5항목)**:

| ID | 검증 내용 | 방법 | 기준 |
|----|----------|------|------|
| G10-1a | block_*.md 파일 수 | Glob + 카운트 | == len(blocks[]) |
| G10-1b | 각 파일 비어있지 않음 | Read | 내용 존재 |
| G10-1c | 🎤 대본 수 하한 | `grep -c '🎤'` | ≥ sessions × 4 × 0.8 |
| G10-1d | 인터리빙 순서 | 첫 🎤 < 첫 `## I Do` | 전체 블록 참 |
| G10-1e | 세션 전수 포함 | `grep -c '# D[0-9]-[0-9]:'` | == len(sessions[]) |

**Step 10-2: 통합 검토 (~15항목)**:

| 영역 | 주요 검증 기준 |
|------|--------------|
| S: 구조 완전성 (S-1~S-7) | §1~§5, §7~§8 존재 (검증 대상: `_header.md`(§1~§3) + `block_D*_{AM\|PM}.md`(§4) + `_footer.md`(§5,§7~§8)), 필수 테이블, 19개 서브섹션, 차시 커버리지 |
...
→ [차시별 교안 작성] session_*.md → [교안 검토] _review_content_*.md
...
- `_review_content_{block_id}.md`: Phase 7 교안 블록별 검토 결과 (블록 수만큼 동적 생성)
...
**슬라이드 유형 (12가지)**:
제목, 아젠다, 섹션전환, 개념설명, 코드, 비교, 데이터+인사이트, 이미지, 실습/활동, 퀴즈/발문, 타임라인, 핵심요약
...
**교안 (`/lecture-script`)** — 4영역 29항목 (Phase 7) + 통합 검토(Phase 10) 2단계:

| 영역 | 항목 수 | 검증 내용 | 검토 단계 |
|------|--------|----------|----------|
| S: 구조 완전성 | 7 | §1~§5, §7~§8 존재 (대상: `_header.md` + `block_*.md` + `_footer.md`), 필수 테이블, 19개 서브섹션, 차시 커버리지 | Phase 10 (통합) |
| G: 구조 완전성 | 5 | 5구간 완전성, GRR 순차, GRR 시간 배분, 확인 활동 삽입 | Phase 7 (블록별) |
| T: 시간 배분 | 8 | 교시 합산, time_ratio ±5%, GRR 비율, 15분 분절, 시간큐 연속성 | Phase 7 (블록별) |
| C: 콘텐츠 정확성 | 12 | CLO/SLO 대조, 소재 근거, 코드 API 정확성(C-9), 코드 밀도(C-10), 플레이스홀더(C-11~C-12) | Phase 7 (블록별) |
| EX: 예시 품질 | 4 | ★ 예시 ≥ 2개(EX-1), You Do 완전성(EX-2), 오개념 반영(EX-4) | Phase 7 (블록별) |
